function [X] = f_ciagla_zlin(t, X)
    ...
    % T = (Fh0*Th +Fc0*Tc + Fd*Td - alpha*sqrt(h0)*T0) / (((pi*h0^2)/3) * (3*r-h0)) + ...d
    %     ((3*alpha)/(h0*sqrt(h0)*(pi*h0-3*pi*r)))*(T-T0) - ...
    %     ((3*Th)/(pi*h0^2*(h0-3*r)))*(Fh - Fh0) - ...
    %     ((3*Tc)/(pi*h0^2*(h0-3*r)))*(Fc - Fc0) + ...
    ...
    T_ = (Fh0*Th + Fc0*Tc + Fd*Td - aqrh0*T0 - (Fh0 + Fc0 + Fd - aqrh0)*T0)/(pi*h0^2*r - pi*h0^3/3) + ...
